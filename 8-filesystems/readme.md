...
* Each node has ***at most*** $m$ elements
* Every node (except the root) has ***at least*** $m/2$ elements
...
2. If the node has fewer than $m$ elements, just insert into the node then return.

3. If the node is full (has $m$ elements), split the node around the middle element into two nodes of $m/2$ elements.
